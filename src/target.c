...
static volatile uint64_t last_sync_pulse_ts_ms;
			      ...
			      uint32_t pins)
{
	...
	last_sync_pulse_ts_ms = (uint64_t)k_uptime_get();
...
void target_run(void)
{
	...
	for (uint32_t cycle = 0U; cycle < EXPERIMENT_SYNC_CYCLES; cycle++) {
		...
		uint64_t pulse_ts_ms;
		uint64_t spi_rx_ts_ms;
		...
		irq_key = irq_lock();
		pulse_ts_ms = last_sync_pulse_ts_ms;
		irq_unlock(irq_key);
		...
		spi_rx_ts_ms = (uint64_t)k_uptime_get();
		controller_ts = decode_timestamp(rx_data);
		spi_path_delay_us = ((int64_t)spi_rx_ts_ms - (int64_t)pulse_ts_ms) * 1000LL;
		...
		if (!have_reference) {
			...
			predicted_worker_ts_ms = ref_worker_ts_ms + (pulse_ts_ms - ref_local_ts_ms);
